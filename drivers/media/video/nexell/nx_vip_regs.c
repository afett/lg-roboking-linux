/* linux/drivers/media/video/snexell/nx_vip_regs.c
 *
 * Register interface file for Nexell NXP2120 camera(VIP) driver
 *
...
void nx_vip_clear_irq(struct nx_vip_control *ctrl)
{
...
static int nx_vip_check_fifo_co(struct nx_vip_control *ctrl)
{
	u32 cfg, status, flag;
...
static int nx_vip_check_fifo_pr(struct nx_vip_control *ctrl)
{
	u32 cfg, status, flag;
...
int nx_vip_check_fifo(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		return nx_vip_check_fifo_pr(ctrl);
	else
		return nx_vip_check_fifo_co(ctrl);
}

void nx_vip_select_camera(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_CIGCTRL);
...
void nx_vip_set_test_pattern(struct nx_vip_control *ctrl, int type)
{
	u32 cfg = readl(ctrl->regs + S3C_CIGCTRL);
...
void nx_vip_set_source_format(struct nx_vip_control *ctrl)
{
...
void nx_vip_set_window_offset(struct nx_vip_control *ctrl)
{
...
static void nx_vip_reset_cfg(struct nx_vip_control *ctrl)
{
	int i;
	u32 cfg[][2] = {
...
void nx_vip_reset(struct nx_vip_control *ctrl)
{
	u32 cfg;
...
void nx_vip_set_polarity(struct nx_vip_control *ctrl)
{
	struct nx_vip_camera *cam = ctrl->in_cam;
	u32 cfg;
...
static void nx_vip_set_rot90(struct nx_vip_control *ctrl)
{
	u32 cfg = 0;
...
static void nx_vip_set_target_format_pr(struct nx_vip_control *ctrl)
{
...
void nx_vip_set_target_format(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_set_target_format_pr(ctrl);
	else
		nx_vip_set_target_format_co(ctrl);
}

static void nx_vip_get_burst_422i(u32 width, u32 *mburst, u32 *rburst)
{
...
static void nx_vip_get_burst(u32 width, u32 *mburst, u32 *rburst)
{
	unsigned int tmp;
...
static void nx_vip_set_output_dma_pr(struct nx_vip_control *ctrl)
{
	struct nx_vip_out_frame *frame = &ctrl->out_frame;
	...
	if (ctrl->rot90) {
		...
		if (frame->format == FORMAT_RGB888)
			nx_vip_get_burst(frame->width * 4, &yburst_m, &yburst_r);
		else
			nx_vip_get_burst(frame->width * 2, &yburst_m, &yburst_r);
	}
...
static void nx_vip_set_output_dma_co(struct nx_vip_control *ctrl)
{
	struct nx_vip_out_frame *frame = &ctrl->out_frame;
	...
	if (frame->format == FORMAT_YCBCR422 && frame->planes == 1) {
		nx_vip_get_burst_422i(frame->width, &yburst_m, &yburst_r);
		cburst_m = yburst_m / 2;
		cburst_r = yburst_r / 2;
	} else {
		nx_vip_get_burst(frame->width, &yburst_m, &yburst_r);
		nx_vip_get_burst(frame->width / 2, &cburst_m, &cburst_r);
	}
...
void nx_vip_set_output_dma(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_set_output_dma_pr(ctrl);
	else
		nx_vip_set_output_dma_co(ctrl);
}

static void nx_vip_enable_lastirq_pr(struct nx_vip_control *ctrl)
{
...
static void nx_vip_enable_lastirq_co(struct nx_vip_control *ctrl)
{
...
void nx_vip_enable_lastirq(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_enable_lastirq_pr(ctrl);
	else
		nx_vip_enable_lastirq_co(ctrl);
}

static void nx_vip_disable_lastirq_pr(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_CIPRCTRL);
...
static void nx_vip_disable_lastirq_co(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_CICOCTRL);
...
void nx_vip_disable_lastirq(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_disable_lastirq_pr(ctrl);
	else
		nx_vip_disable_lastirq_co(ctrl);
}

static void nx_vip_set_prescaler_pr(struct nx_vip_control *ctrl)
{
...
static void nx_vip_set_prescaler_co(struct nx_vip_control *ctrl)
{
	struct nx_vip_scaler *sc = &ctrl->scaler;
	u32 cfg = 0, shfactor;
...
void nx_vip_set_prescaler(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_set_prescaler_pr(ctrl);
	else
		nx_vip_set_prescaler_co(ctrl);
}

static void nx_vip_set_scaler_pr(struct nx_vip_control *ctrl)
{
	struct nx_vip_scaler *sc = &ctrl->scaler;
...
static void nx_vip_set_scaler_co(struct nx_vip_control *ctrl)
{
	struct nx_vip_scaler *sc = &ctrl->scaler;
...
void nx_vip_set_scaler(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_set_scaler_pr(ctrl);
	else
		nx_vip_set_scaler_co(ctrl);
}

static void nx_vip_start_scaler_pr(struct nx_vip_control *ctrl)
{
...
static void nx_vip_start_scaler_co(struct nx_vip_control *ctrl)
{
...
void nx_vip_start_scaler(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_start_scaler_pr(ctrl);
	else
		nx_vip_start_scaler_co(ctrl);
}

static void nx_vip_stop_scaler_pr(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_CIPRSCCTRL);
...
static void nx_vip_stop_scaler_co(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_CICOSCCTRL);
...
void nx_vip_stop_scaler(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_stop_scaler_pr(ctrl);
	else
		nx_vip_stop_scaler_co(ctrl);
}

void nx_vip_enable_capture(struct nx_vip_control *ctrl)
{
...
void nx_vip_disable_capture(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_CIIMGCPT);
...
void nx_vip_set_effect(struct nx_vip_control *ctrl)
{
	struct nx_vip_effect *effect = &ctrl->out_frame.effect;
...
static void nx_vip_set_input_dma_size_pr(struct nx_vip_control *ctrl)
{
...
static void nx_vip_set_input_dma_pr(struct nx_vip_control *ctrl)
{
	struct nx_vip_in_frame *frame = &ctrl->in_frame;
	u32 cfg;

	/* for original & real size */
	nx_vip_set_input_dma_size_pr(ctrl);
...
static void nx_vip_set_input_dma_size_co(struct nx_vip_control *ctrl)
{
	struct nx_vip_in_frame *frame = &ctrl->in_frame;
	int ofs_h = frame->offset.y_h * 2;
	int ofs_v = frame->offset.y_v * 2;
...
static void nx_vip_set_input_dma_co(struct nx_vip_control *ctrl)
{
	struct nx_vip_in_frame *frame = &ctrl->in_frame;
	u32 cfg;

	/* for original & real size */
	nx_vip_set_input_dma_size_co(ctrl);
...
void nx_vip_set_input_dma(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_set_input_dma_pr(ctrl);
	else
		nx_vip_set_input_dma_co(ctrl);
}

static void nx_vip_start_input_dma_pr(struct nx_vip_control *ctrl)
{
...
static void nx_vip_start_input_dma_co(struct nx_vip_control *ctrl)
{
...
void nx_vip_start_input_dma(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_start_input_dma_pr(ctrl);
	else
		nx_vip_start_input_dma_co(ctrl);
}

static void nx_vip_stop_input_dma_pr(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_MSPRCTRL);
...
static void nx_vip_stop_input_dma_co(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_MSCOCTRL);
...
void nx_vip_stop_input_dma(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_stop_input_dma_pr(ctrl);
	else
		nx_vip_stop_input_dma_co(ctrl);
}

static void nx_vip_set_input_path_pr(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_MSPRCTRL);
...
static void nx_vip_set_input_path_co(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_MSCOCTRL);
...
void nx_vip_set_input_path(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_set_input_path_pr(ctrl);
	else
		nx_vip_set_input_path_co(ctrl);
}

static void nx_vip_set_output_path_pr(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_CIPRSCCTRL);
...
static void nx_vip_set_output_path_co(struct nx_vip_control *ctrl)
{
	u32 cfg = readl(ctrl->regs + S3C_CICOSCCTRL);
...
void nx_vip_set_output_path(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1)
		nx_vip_set_output_path_pr(ctrl);
	else
		nx_vip_set_output_path_co(ctrl);
}

void nx_vip_set_input_address(struct nx_vip_control *ctrl)
{
	struct nx_vip_in_frame *frame = &ctrl->in_frame;
	struct nx_vip_frame_addr *addr = &ctrl->in_frame.addr;
...
static void nx_vip_set_output_address_pr(struct nx_vip_control *ctrl)
{
	struct nx_vip_out_frame *frame = &ctrl->out_frame;
...
static void nx_vip_set_output_address_co(struct nx_vip_control *ctrl)
{
	struct nx_vip_out_frame *frame = &ctrl->out_frame;
	struct nx_vip_frame_addr *addr;
	int i;
...
{
	if (ctrl->id == 1)
		nx_vip_set_output_address_pr(ctrl);
	else
		nx_vip_set_output_address_co(ctrl);
}

int nx_vip_get_frame_count(struct nx_vip_control *ctrl)
{
	if (ctrl->id == 1) {
...
void nx_vip_change_effect(struct nx_vip_control *ctrl)
{
	struct nx_vip_effect *effect = &ctrl->out_frame.effect;
...
void nx_vip_change_rotate(struct nx_vip_control *ctrl)
{
	u32 cfg;
