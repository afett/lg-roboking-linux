...
#define	_LOCK(id)		{ if (! in_interrupt()) mutex_lock(&mutex);   }
#define	_UNLOCK(id)		{ if (! in_interrupt()) mutex_unlock(&mutex); }
...
void soc_pwm_init(void)
{
	...
	printk(KERN_INFO "pwm frequency range: %lu hz ~ %lu hz, pll:%lu\n",
...
void soc_pwm_set_frequency(int ch, u_int freq, u_int duty)
{
	...
	__ASSERT(PWM_CHANNEL_NUM > ch && ch >= 0);
